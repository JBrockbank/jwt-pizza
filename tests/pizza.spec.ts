...
test('create store with login', async ({ page }) => {
  ...
  await page.route('http://localhost:3000/api/franchise?page=0&limit=3&name=*', async (route) => {
    await route.fulfill({
      status: 200,
      json: {
        franchises: [
  ...
  await page.route('http://localhost:3000/api/franchise', async (route) => {
    ...
    await route.fulfill({
      status: 201,
      json: { id: 123, name: postData.name, stores: [] },
    });
  });

  await page.route('http://localhost:3000/api/franchise?page=0&limit=10&name=*new*', async (route) => {
    await route.fulfill({
      status: 200,
      json: {
        franchises: [
          { id: 123, name: 'newTest', stores: [] },
        ],
        more: false,
      },
    });
  ...
  await page.route('http://localhost:3000/api/franchise/18', async (route) => {
